                ...
                if (frontText) {
                    frontText.innerHTML = `
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px; align-items: center; width: 100%;">
                            ...
                            <button class="action-btn" onclick="app.setMode('manage-review')" style="width: 100%; max-width: 220px; padding: 12px; font-size: 1rem; background: transparent; color: var(--text-muted); border: 1px solid var(--border-color);"> Manage Review List</button>
                        </div>
                    `;
                }
...
document.addEventListener('DOMContentLoaded', () => {
    const cardSizer = new MutationObserver(() => {
        const targetBoxes = document.querySelectorAll('#fc-front-text, .char-hz, #fc-back-word');
        
        targetBoxes.forEach(box => {
            // Get just the raw characters to count them accurately
            const rawText = box.innerText.replace(/[\s\p{P}]/gu, '');
            
            // Prevent lag by only running when the text actually changes
            if (box.dataset.frontResized === rawText) return; 
            
            const charCount = rawText.length;
            if (charCount === 0) return;

            // 🚨 1. ALIGNMENT & WRAPPING RULES
            box.style.setProperty('display', 'block', 'important');
            box.style.setProperty('width', '100%', 'important');
            
            if (box.id === 'fc-back-word') {
                // Back card: Allow long phrases to wrap so they fit the card
                ...
                // Front card: Force onto a single line
                box.style.setProperty('white-space', 'nowrap', 'important');
                box.style.setProperty('word-break', 'keep-all', 'important');
            }

            // 🚀 2. DETERMINE THE PERFECT SIZE
            let newSize = '';
            if (charCount <= 2) {
                newSize = 'clamp(4rem, 15vw, 8rem)'; // Massive for words
            } else if (charCount <= 4) {
                newSize = 'clamp(3rem, 12vw, 6rem)'; // Large for idioms
            } else if (charCount <= 6) {
                newSize = 'clamp(2rem, 8vw, 4rem)'; // Medium for short phrases
            } else {
                newSize = 'clamp(1.5rem, 8vw, 3rem)'; // Shrink for long sentences
            }

            // 🎯 3. THE BULLETPROOF FIX: Force both the box AND every letter inside to grow!
            box.style.setProperty('font-size', newSize, 'important');
            box.style.setProperty('line-height', '1.2', 'important');
            
            // Find every single span/character inside and force the size on them directly
            ...
            // Remember this text so we don't recalculate it unnecessarily
            box.dataset.frontResized = rawText;
        });
    });
