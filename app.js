...
buildSentencesUI(item) {
    try {
        ...
        // 🗑️ 1. KILL ALL SCROLL BUTTONS
        const scrollDownBtn = document.querySelector('.scroll-down-hint');
        const scrollUpBtn = document.querySelector('.scroll-up-hint'); 
        
        if (scrollDownBtn) scrollDownBtn.style.setProperty('display', 'none', 'important');
        if (scrollUpBtn) scrollUpBtn.style.setProperty('display', 'none', 'important');


      const scrollArea = document.querySelector('.flashcard-back') || document.querySelector('.card-back');
        if (scrollArea) {
            // We ONLY set the CSS. The main flashcard script now handles the math!
            ...
            // 🌟 THE NEW FIX: Instantly snap the scrollbar back to the very top for the new card!
            scrollArea.scrollTop = 0; 
        }

        // 🛑 Prevent card flip when just tapping the sentences area
        container.onclick = (e) => e.stopPropagation();

        // 🛑 Prevent card flip when just tapping the sentences area
        container.onclick = (e) => e.stopPropagation();
        // 🛑 3. Prevent card flip when tapping the sentences area
        container.onclick = (e) => e.stopPropagation();
        ...
        // 🎯 4. NO BOXES! Text directly on the card
