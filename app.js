...
// ==========================================
  // ==========================================
    // 🎯 LASER-FOCUSED SENTENCES BUILDER (CURRENT LESSON ONLY)
    // ==========================================
buildSentencesUI(item) {
        try {
            ...
            // 🛑 STOP SCROLLING/TOUCHING FROM FLIPPING THE CARD
            ...
            if (!targetWord) {
                container.innerHTML = '';
                if (oldBox) oldBox.style.display = 'block'; 
                return;
            }

            const currentBook = `B${item.book_id}`; 
            const currentLesson = String(item.lesson_id).padStart(2, '0'); 
            let matchingSentences = [];
            
            // 🔍 SCAN DATABASE FOR ALL OCCURRENCES ACROSS ALL BOOKS
                ...
                window.sentences.forEach(s => {
                    ...
                    if (zh.includes(targetWord)) {
                        if (!matchingSentences.some(m => String(m.sentence) === zh)) {
                            ...
                            // Check if this sentence is from the card's exact current lesson
                            const isCurrent = (s.book_id === currentBook && s.lesson_id === currentLesson);
            ...
            // 🔀 SORT RESULTS: Pin the current lesson's sentence to the very top!
            ...
            // 🛡️ FALLBACK: If no sentences found, show the old box
            ...
            // 🎯 SUCCESS! HIDE the old box so they don't duplicate!
            if (oldBox) oldBox.style.display = 'none';

            // 🏷️ BUILD THE UI 
            let sentencesHTML = matchingSentences.map(match => {
                
                // Style the badge differently if it's the current lesson vs an older/future lesson
                const badgeColor = match.isCurrentLesson 
                    ? "color: #0369a1; background: #e0f2fe; border: 1px solid #bae6fd;" 
                    : "color: #4b5563; background: #f3f4f6; border: 1px solid #e5e7eb;";
                const currentLabel = match.isCurrentLesson ? " ⭐ Current Context" : "";

                return `
                <div class="fc-sentence-item" style="padding-bottom: 15px; margin-bottom: 15px; border-bottom: 1px solid var(--border-color, #eee);">
                    <div style="display: inline-block; font-size: 0.75rem !important; font-weight: bold; ${badgeColor} padding: 4px 10px; border-radius: 20px; margin-bottom: 10px;">
                        🏷️ ${match.book} - ${match.lesson}${currentLabel}
                    </div>
                    <div class="text-chinese" style="font-size: 1.3rem !important; white-space: normal !important; word-wrap: break-word !important; color: var(--primary-color); line-height: 1.4 !important;">${match.sentence}</div>
                    <div class="text-pinyin" style="font-size: 0.95rem !important; white-space: normal !important; word-wrap: break-word !important; color: var(--text-muted); margin-bottom: 5px;">${match.pinyin}</div>
                    <div class="text-english" style="font-size: 0.95rem !important; white-space: normal !important; word-wrap: break-word !important;">${match.english}</div>
                </div>
                `;
            }).join('');
            ...
            container.innerHTML = `
                <div class="fc-sentences-wrapper" style="margin-top: 15px; padding: 15px; background: var(--card-bg, #fff); border-radius: 12px; border: 1px solid var(--border-color, #eee);">
                    <div style="font-weight: bold; color: #0369a1; margin-bottom: 15px; border-bottom: 1px solid #e0f2fe; padding-bottom: 8px; font-size: 1rem !important;">
                        📚 ${matchingSentences.length} Context Sentence(s)
                    </div>
                    ${sentencesHTML.replace(/border-bottom: 1px solid var\(--border-color, #eee\);$/, 'border-bottom: none; margin-bottom: 0; padding-bottom: 0;')}
                </div>
            `;
        } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    const cardSizer = new MutationObserver(() => {
        ...
        targetBoxes.forEach(box => {
            ...
            if (box.id === 'fc-back-word') {
                if (charCount <= 2) newSize = 'clamp(3rem, 16vw, 4rem)'; 
                else if (charCount <= 4) newSize = 'clamp(2.2rem, 11vw, 3rem)'; 
