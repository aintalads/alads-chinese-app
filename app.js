class ChineseApp {
   ...
   loadActiveSession() {
        ...
        try {
            ...
            // 🚀 1. ADD THIS HERE (For returning users)
...
showSessionComplete() {
        ...
        // PRIMARY: Try to read from UI counters
        ...
        // FALLBACK: If UI is 0, use the App's internal memory (this.state)
        // This is the most common reason for the "Static 0%" bug
        if (totalCards === 0) {
            knownCards = this.state.score || 0;
            // Calculate total from the history of swiped cards
        ...
        if (totalCards === 0) totalCards = 1; // Safety net
        ...
        // ==========================================
        // 2. LINE GRAPH: Data Mapping & Label Fix
        // ==========================================
        const sessionData = [72, 78, 81, 84, accuracy]; 
        const finalScore = accuracy;
        const previousScore = sessionData[3];

        // FIX: Update the floating label on the line graph immediately
        const graphLabel = document.getElementById('sc-graph-label');
        if (graphLabel) {
            graphLabel.innerText = `${finalScore}%`; 
        }

        // Improvement Badge Logic
        ...
        if (badge) {
            ...
            } else if (finalScore < previousScore) {
        ...
        // Generate SVG Graph Paths
        const graphWidth = 300;
        const graphHeight = 100;
        const minVal = Math.max(0, Math.min(...sessionData) - 5);
        const range = 100 - minVal || 1; 

        const points = sessionData.map((d, i) => ({
            x: (i / (sessionData.length - 1)) * graphWidth,
            y: graphHeight - ((d - minVal) / range) * graphHeight
        }));
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            d += `C ${cpX},${pPrev.y} ${cpX},${p.y} ${p.x},${p.y} `;
        }
        const areaPath = `${d} L ${graphWidth},${graphHeight} L 0,${graphHeight} Z`;
        ...
        if (lastPointEl) {
            ...
            lastPointEl.style.transform = `translate(${points[points.length-1].x}px, ${points[points.length-1].y}px)`;
        }
        ...
        setTimeout(() => {
            ...
            // C) Draw Line Graph Animation
            if (lineEl) {
                lineEl.style.transition = 'stroke-dashoffset 1.5s ease-in-out';
                lineEl.style.strokeDashoffset = "0";
            }
            if (areaEl) {
                areaEl.style.transition = 'opacity 1s ease-in-out 0.5s';
                areaEl.style.opacity = "1";
            }
            setTimeout(() => {
                if (lastPointEl) {
                    lastPointEl.classList.remove('hidden');
                    lastPointEl.style.transition = 'opacity 0.5s ease-out, transform 0.5s ease-out';
...
document.addEventListener('DOMContentLoaded', () => {
    const cardSizer = new MutationObserver(() => {
        ...
        targetBoxes.forEach(box => {
            ...
            const rawText = text.replace(/[\s\p{P}]/gu, '');
            ...
            if (!hasChinese) {
                // Remove the giant rules from the main box container
                ...
                // 🚨 THE FIX: We STOP here! We do not strip the inner elements anymore.
                // This allows the "Session Complete!" text and emoji to keep their perfect sizes!
            ...
            if (box.id === 'fc-back-word') {
                // Back Card Sizes
                if (charCount <= 2) {
                    newSize = 'clamp(3rem, 16vw, 4rem)'; 
                } else if (charCount <= 4) {
                    newSize = 'clamp(2.2rem, 11vw, 3rem)'; 
                } else if (charCount <= 6) {
                    newSize = 'clamp(1.8rem, 7vw, 2rem)'; 
                } else {
                    newSize = 'clamp(1.3rem, 6vw, 1.5rem)'; 
                }
            } else {
                // Front Card Sizes
                if (charCount <= 2) {
                    newSize = 'clamp(4rem, 20vw, 6rem)'; 
                } else if (charCount <= 4) {
                    newSize = 'clamp(3rem, 15vw, 4.5rem)'; 
                } else if (charCount <= 6) {
                    newSize = 'clamp(2rem, 8vw, 2rem)'; 
                } else {
                    newSize = 'clamp(1.5rem, 8vw, 3rem)'; 
                }
            }
    ...
    // 🎯 3. APPLY THE SIZES TO THE BOX AND INNER LETTERS
            ...
            innerLetters.forEach(letter => {
                ...
                // 🛑 CLEAN SHIELD: Simply ignore the example box and let CSS handle it!
                if (letter.closest('.extra-info-box') || letter.closest('.fc-ex-zh')) {
                    return; 
                }
